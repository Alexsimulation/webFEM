...
function eval_math(uf, vf, x, expr) {
    var ux = grad(uf)(x);
    var vx = grad(vf)(x);
    var u = uf(x);
    var v = vf(x);
...
function eval_weak(phi0, phi1, x0, x1, bilinear, linear) {
    ...
    function f(u, v, expr) {
        function f_(x) { 
            return eval_math(u, v, x, expr); 
        }
        return f_;
    }

    var u = phi0; var v = phi0;
    b00 = integrate(f(u, v, bilinear), x0, x1);

    var u = phi0; var v = phi1;
    b01 = integrate(f(u, v, bilinear), x0, x1);

    var u = phi1; var v = phi0;
    b10 = integrate(f(u, v, bilinear), x0, x1);

    var u = phi1; var v = phi1;
    b11 = integrate(f(u, v, bilinear), x0, x1);

    var v = phi0;
    l0 = integrate(f(u, v, linear), x0, x1);

    var v = phi1;
    l1 = integrate(f(u, v, linear), x0, x1)
...
function gen_system(bilinear, linear, bc0, bc1, x) {
